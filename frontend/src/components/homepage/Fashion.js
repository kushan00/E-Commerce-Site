...
<div class="input-group">
  ...
  <input type="search" class="form-control rounded" placeholder="Search Electronics" aria-label="Search" onChange={event=>{setserachItem(event.target.value)}} 
   ...
   <br></br><br></br>
      <h1 className="text-center mt-3">All Fashion items List</h1>
    
 
      {Sposts.filter((Fashion)=>{
    if(serachItem==""){
      return Fashion
    }else if(Fashion.SitemName.toLowerCase().includes(serachItem.toLowerCase())){
      return Fashion
    }
  })
...
<div className="card p-0 overflow-hidden h-100 shadow" >
  ...
  <h4 className="card-title">&nbsp; {Fashion.SitemName} </h4>
  <p className="card-title">&nbsp;Brand Name: {Fashion.SbrandName} </p>
  <p className="card-text">&nbsp; Unit price: {Fashion.SunitPrice}</p>
  <p className="card-text">&nbsp;category: {Fashion.Scategory}</p>
  <p className="card-text">&nbsp;Shipping cost: {Fashion.SshippingCost}</p>
  <p className="card-text">&nbsp;Qunatity available: {Fashion.SquntityAvailable}</p>
