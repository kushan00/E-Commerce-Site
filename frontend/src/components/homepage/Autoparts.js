...
<div class="input-group">
  ...
  <input type="search" class="form-control rounded" placeholder="Search Electronics" aria-label="Search" onChange={event=>{setserachItem(event.target.value)}} 
   ...
   <br></br><br></br>
      <h1 className="text-center mt-3">All Auto parts List</h1>
    
 
      {Aposts.filter((Autoparts)=>{
    if(serachItem==""){
      return Autoparts
    }else if(Autoparts.AitemName.toLowerCase().includes(serachItem.toLowerCase())){
      return Autoparts
    }
  })
...
<div className="card p-0 overflow-hidden h-100 shadow" >
  ...
  <h4 className="card-title">&nbsp; {Autoparts.AitemName} </h4>
  <p className="card-title">&nbsp;Brand Name: {Autoparts.AbrandName} </p>
  <p className="card-text">&nbsp; Unit price: {Autoparts.AunitPrice}</p>
  <p className="card-text">&nbsp;category: {Autoparts.Acategory}</p>
  <p className="card-text">&nbsp;Shipping cost: {Autoparts.AshippingCost}</p>
  <p className="card-text">&nbsp;Qunatity available: {Autoparts.AquntityAvailable}</p>
