...
<div class="input-group">
  ...
  <input type="search" class="form-control rounded" placeholder="Search Electronics" aria-label="Search" onChange={event=>{setserachItem(event.target.value)}} 
   ...
   <br></br><br></br>
      <h1 className="text-center mt-3">All Garden items List</h1>
    
 
      {Gposts.filter((Garden)=>{
    if(serachItem==""){
      return Garden
    }else if(Garden.GitemName.toLowerCase().includes(serachItem.toLowerCase())){
      return Garden
    }
  })
...
<div className="card p-0 overflow-hidden h-100 shadow" >
  ...
  <h4 className="card-title">&nbsp; {Garden.GitemName} </h4>
  <p className="card-title">&nbsp;Brand Name: {Garden.GbrandName} </p>
  <p className="card-text">&nbsp; Unit price: {Garden.GunitPrice}</p>
  <p className="card-text">&nbsp;category: {Garden.Gcategory}</p>
  <p className="card-text">&nbsp;Shipping cost: {Garden.GshippingCost}</p>
  <p className="card-text">&nbsp;Qunatity available: {Garden.GquntityAvailable}</p>
